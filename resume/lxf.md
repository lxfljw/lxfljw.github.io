...
- 具备 AI 研发工程化实践，自研并推广团队级 dev-flow MCP。
...
- 负责美业 SaaS 全栈业务开发，覆盖 CRM、用户权益、标签分层及运营触达等核心场景，主导 CRM 工程级定制化方案落地。
- 推进 Yarn 向 pnpm 迁移、Node 升级及 CI 流程适配等工程效能治理。
- 主导 dev-flow MCP Server 建设，将日志、DB、接口和发布能力接入 AI 研发工作流。
...
- 负责 3 名前端的任务拆解、排期、Code Review、技术方案及交付风险管理。
- 负责零售云与营销 SaaS 核心模块开发，主导 Web 与微信小程序技术栈升级（Vue 1 → Vue 3、WePY → UniApp、Taro 2 → Taro 3）。
- 建设组件库、小程序非侵入式埋点、动态主题等通用能力，推进工程化基线与小程序 DevOps 建设。
...
- 负责秦丝 SaaS 进销存系统 Web、小程序及混合 App 业务开发。
...
- **小红书小程序**：在 2 个月内基于 UniApp 独立完成前端从 0 到 1 建设，覆盖商品配置、详情、支付及订单等核心交易页面；参与 Spring Boot 后端开发，负责商品提审、POI 绑定等能力，打通商品配置、交易下单与到店核销链路。
...
- **AI 研发工作流（dev-flow MCP）**：面向线上问题排查与修复，主导建设团队级 AI Native 研发基础设施。将日志、数据库、内部接口与发布能力抽象为 MCP 工具并接入 IDE，沉淀以 TraceId 为入口的跨系统诊断工作流，并建立 SQL 风险分析、写操作拦截与调用审计等安全治理能力。推广至团队约 **30 人**使用，将原先依赖多系统人工排查的流程收敛为 IDE 内一站式诊断。
...
- **通用能力沉淀**：参与业务组件库建设，负责多级联动动态筛选组件实现；设计小程序非侵入式代理埋点与动态主题方案，实现数据采集、主题配置与业务逻辑解耦；将原生 Canvas 海报迁移至开源框架并参与缺陷修复。
...
**主要技术栈**：Vue 2、Webpack、Element UI、微信原生小程序

- **小程序首页装修**：参与首页装修系统从 0 到 1 建设，面向商户自主配置小程序首页，支持十余种装修组件；参与设计 Schema 配置结构与动态渲染机制，打通 B 端配置、实时预览及微信小程序渲染链路。
